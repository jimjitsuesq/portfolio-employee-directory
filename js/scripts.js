...
function search () {
    ...
    for (let i = 0; i < employeeCards.length; i++) {
        const searchTarget = employeeCards[i].children[0].childNodes[2].nextElementSibling.childNodes[1];
        if ((searchTarget.textContent.toLowerCase()).includes(searchInput.value.toLowerCase())) {
            gallery.appendChild(employeeCards[i]);
        }
    }
